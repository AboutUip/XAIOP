...
> **Sealed archive** — [README.md](README.md) · [SEAL.md](SEAL.md).
...
| Depends on | Metrics snapshot protocol **0.2.0** (historical) · Node.js SDK compatibility mode; tip at seal protocol **0.6.0** (historical benches vs 0.5.0) |
...
1. **Native dual channel** — For **LLM bench metrics**, JSON and XAIOP are generated separately; the evaluation **must not** score a model by translating JSON→XAIOP after the fact.  
   *(The Node.js SDK **does** ship `encode` / `uploadJson` for tools, tests, and adapters — see [sdk/nodejs/API.md](../../sdk/nodejs/API.md). That API is out of scope for these dual-channel model scores.)*  
...
**Product stance:** gains are **conditional on model profile** (often larger when JSON baseline is weaker; can be near-zero gain with higher cost when JSON is already strong). Full narrative — problem framing, seven-bench pattern, bottlenecks outside the protocol, current positioning: **[overview/introduction.md](../../overview/introduction.md)**.

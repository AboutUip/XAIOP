...
> **目标封存** — [README.zh-CN.md](README.zh-CN.md) · [SEAL.md](SEAL.md)。
...
| 最近更新 | 2026-08-03 |
| 是否规范性 | **否** — 评测口径与已发布快照 |
| 依赖 | 指标快照协议 **0.2.0**（历史）· Node.js SDK 兼容模式；封存时 tip 协议 **0.6.0**（历史评测曾对 0.5.0） |
...
1. **原生双通道** — 在 **LLM 评测指标**中，JSON 与 XAIOP 各自生成；评测**不得**在事后把 JSON 转写成 XAIOP 再计分。  
   *（Node.js SDK **提供** `encode` / `uploadJson`，供工具、测试与适配使用 — 见 [sdk/nodejs/API.zh-CN.md](../../sdk/nodejs/API.zh-CN.md)。该 API 不计入上述双通道模型得分。）*  
...
**产品定位：** 收益**取决于模型能力画像**（JSON 基线越弱，结构提升往往越大；JSON 已经很强时，结构提升可能趋近于零，但成本更高）。完整叙事——问题定义、七场基准规律、协议外瓶颈、当前立场：见 **[overview/introduction.zh-CN.md](../../overview/introduction.zh-CN.md)**。

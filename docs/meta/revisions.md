...
| Node.js SDK | **0.11.0** — ignore `#` lines; `PROTOCOL_VERSION` → `0.6.0`. Tip **0.12.0** — buffer line intercept (`onLineIntercept`). Tip **0.13.0** — Annotation Span (`onAnnotationSpan`; typeCheck escape). Tip **0.14.0** — Control Root `#!` demux / session / seq / resume. Tip **0.14.1** — `#!xaiop/seq/v1` → `meta.logSeq`. Tip **0.14.2** — Diff isolation after `.` (D1). Tip **0.14.3** — `@` cumulative Diff (D2); optional `onChunk`. Tip **0.15.0** — `bufferStats` / `compactCommitted` (still protocol **0.6.0**) |

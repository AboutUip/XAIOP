...
Content **行**的值中 **不得**含物理 `LF` / `CRLF`。下一物理行是新的解析尝试，绝不是值续行。空 Content 行仍为语法错误（第 7 节）。

字符串值 **可以**含 `U+000A` 与 `U+000D`。这些码点在线上 **必须**写成第 4.1 节的两字符转义。本字母表 **一律生效**（不是选项）。

### 4.1 Content 转义字母表

按第一个 `:` 切开、剥掉强制 string 前导空格（第 6 节）之后，对剩余载荷 **先解转义再打字**（第 5 节）。

若载荷不含 `U+005C REVERSE SOLIDUS`（`\`），解转义为空操作。
...
Encode **必须**对字符串值中的每个 `\` / `U+000A` / `U+000D` 应用本字母表（第 6 节前导空格拒绝之后）。

### 4.2 与包 0.6.0 的关系

协议包 **0.6.0** 把载荷里的 `\n` / `\r` / `\\` 当字面字符。包 **0.7.0** 会解开它们。引用时写明包版本。

---

## 5. 最小类型化

在强制字符串标记（第 6 节）与解转义（第 4.1 节）之后，按**首个匹配**规则：
...
**编码器含义：** 首字符为 U+0020 SPACE 的 JSON 字符串无法在 `:` 后无损落线（这些空格即本标记）。符合的 encode API **必须**拒绝此类值，而不是发出 parse 后语义不同的线文。前导 Tab（`U+0009`）不是本标记，可以往返。含 `U+000A` / `U+000D` 的 JSON 字符串 **必须**用第 4.1 节转义，不得用物理换行。

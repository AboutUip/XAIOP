...
| 路径 | 作用 |
| --- | --- |
| `fixtures/` | 共享 `.xaiop` / JSON 语料（Node ↔ Java） |
...
`.github/workflows/ci.yml` 任务：`node`、`java`、`python`、`go`、`golden`（Node↔Java）、`core-wire`（Python↔Go）、`fuzz`。

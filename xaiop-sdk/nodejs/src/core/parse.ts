...
class Parser {
  ...
  deleteAbsolute(segments) {
    ...
    const root = /** @type {Record<string, unknown>} */ (this.root);
    this.deleteFromObject(root, segments, /*relative*/ false);
  }
  ...
  deleteRelative(segments) {
    ...
    const obj = /** @type {Record<string, unknown>} */ (cur.value);
    this.deleteFromObject(obj, segments, /*relative*/ true);
  }
  ...
  precheckRelativeDelete(segments) {
    ...
    let obj = /** @type {Record<string, unknown>} */ (cur.value);
    for (let i = 0; i < segments.length; i++) {
      const seg = segments[i];
  ...
  deleteFromObject(start, segments, _relative) {
    ...
    // Forbid deleting the document root value itself (defensive).

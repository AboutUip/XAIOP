...
describe("control demux — edge cases", () => {
  ...
  test("dispatchControlFrame parseControlBodyJson empty → null", () => {
...
describe("phase seq — more coverage", () => {
...
describe("WS producer outbound + resume replay", () => {
  ...
  test("cross-reconnect resume with app-owned ResumeWireLog", async () => {
    ...
    // Reconnect: resume from seq 1 → only b + c (and optional snapshot).
    {
      const phases = [];
      const snapshots = [];
      const client = await XaiopWs.connect(hub.url(), {
